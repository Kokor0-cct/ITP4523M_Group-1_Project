...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['coid'])) {
    $delOid = intval($_GET['coid']);

    $checkSql = "SELECT coid, cid, codeliveraddress, costatus, cototalamount FROM customeorders WHERE coid = ?";
...
$allowedSortFields = ['coid', 'coid', 'cototalamount', 'codeliverydate', 'costatus'];
        ...
        <table border="1" width="100%">
            <thead>
                <tr>
                    ...
                    <th class="sortable-th" onclick="window.location.href='customorder.php?sortBy=odate&sortDir=<?= $sortBy == 'codate' ? $toggleDir : 'asc' ?>'">
                        Order Date
                        <?php if($sortBy == 'codate'): ?>
                            <span class="sort-arrow"><?= $sortDir == 'asc' ? '↑' : '↓' ?></span>
                        <?php endif; ?>
                    </th>
                    <th class="sortable-th" onclick="window.location.href='customorder.php?sortBy=ototalamount&sortDir=<?= $sortBy == 'cototalamount' ? $toggleDir : 'asc' ?>'">
                        Total Amount
                        <?php if($sortBy == 'cototalamount'): ?>
                            <span class="sort-arrow"><?= $sortDir == 'asc' ? '↑' : '↓' ?></span>
                        <?php endif; ?>
                    </th>
                    <th class="sortable-th" onclick="window.location.href='customorder.php?sortBy=odeliverydate&sortDir=<?= $sortBy == 'codeliverydate' ? $toggleDir : 'asc' ?>'">
                        Delivery Date
                    ...
                    <th class="sortable-th" onclick="window.location.href='customorder.php?sortBy=ostatus&sortDir=<?= $sortBy == 'costatus' ? $toggleDir : 'asc' ?>'">
                        Order Status
